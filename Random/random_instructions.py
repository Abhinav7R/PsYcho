...
def welcome_block_screen():
    """Display welcome screen for block version"""
    welcome_text = """Level 3"""
    
    display_text_until_space(welcome_text, text_color)

def block_explanation():
    """Explain the block format"""
    explanation_text = """How this level works:
    Again, three types of stimuli will be presented to you.

    Direction : Where the arrow head is pointing. 
    Movement : Where the object is moving.
    Sound : In which ear beep is played.
    <highlight>You have to respond based on the BACKGROUND COLOR.</highlight>
    """
    
    display_colored_text_until_space(explanation_text, text_color)

def block_back_color_explanation():
    """Explain neutral block"""
    neutral_text = """
        Press <highlight>the left or right arrow keys</highlight> based on the stimuli.
        Use both hands for left and right keys.

        Note : The background color would be:
        <green>Green</green> : Sound
        <red>Red</red> : Movement
        <blue>Blue</blue> : Direction
        """
...
This is where the block experiment
